...
class EmitterBase {
  ...
  constructor() {
    if (new.target === EmitterBase) {
      throw new TypeError("Cannot construct EmitterBase instances directly");
    }

    if (this.getNestedElementAccessor === undefined) {
      throw new TypeError("Must override method getNestedElementAccessor");
    }

    if (this.semicolonClosesMethodBody === undefined) {
      throw new TypeError("Must override method semicolonClosesMethodBody");
    }

  }

  isNestedElement(traverser) {
    return traverser.element.getAncestorsOfType(NG_FOR, NG_REPEAT).length > 0;
  }
  ...
  referenceToElement(traverser, addIndexForRepeater = true) {
    ...
    if (addIndexForRepeater && traverser.element.hasAnyType(NG_FOR, NG_REPEAT)) {
      let repeaterCount = traverser.element.getAncestorsOfType(NG_FOR, NG_REPEAT).length;
      selector += this.getNestedElementAccessor(`rowIndex${repeaterCount + 1}`);
    }
  ...
  methodBody(traverser, bodyLines) {
    ...
    result += '}';
    result += this.semicolonClosesMethodBody() ? ';' : '';
    result += traverser.newLine();
