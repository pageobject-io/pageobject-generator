...
class RadioButtonProcessor {
  ...
  process(context) {
    let elementsInGroup = findElementsInGroup(context);
    if (elementsInGroup[0] === context.domElement) {
      let pageElement = context.addPageElement();

      for (var i = 1; i < elementsInGroup.length; i++) {
        pageElement.addPosition(elementsInGroup[i]);
      }

      if (elementsInGroup.length == 1) {
        pageElement.addTypes(Types.REPEATED_RADIO);
      } else {
