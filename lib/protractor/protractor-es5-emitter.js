...
class ProtractorEs5Emitter extends EmitterBase {
  ...
  emitEnabledAssertion(traverser) {
    ...
    let args = [];
    let result = this.methodHeader(traverser, name, args);
    result += super.methodBody(traverser, `expect(${super.referenceToElement(traverser)}.isEnabled()).toBe(true);`);
    result += traverser.newLine();

    name = `${traverser.element.name}ShouldNotBeEnabled`;
    result += this.methodHeader(traverser, name, args);
    result += super.methodBody(traverser, `expect(${super.referenceToElement(traverser)}.isEnabled()).toBe(false);`);
  ...
  emitEnabledByIndexAssertion(traverser) {
    ...
    let result = this.methodHeader(traverser, name, args);
    result += super.methodBody(traverser,
                               `expect(${super.referenceToElement(traverser)}.get(index).isEnabled()).toBe(true);`);
    result += traverser.newLine();

    name = `${traverser.element.name}ByIndexShouldNotBeEnabled`;
    result += this.methodHeader(traverser, name, args);
    result += super.methodBody(traverser,
                               `expect(${super.referenceToElement(traverser)}.get(index).isEnabled()).toBe(false);`);
  ...
  emitHasClassAssertion(traverser) {
    ...
    let body = [`${super.referenceToElement(traverser)}.getAttribute('class').then(function (classes) {`,
                `${traverser.singleIndent()}expect(classes.split(' ').indexOf(className) !== -1).toBe(true);`,
                '});'];
  ...
  emitOptionSelectedByPartialTextAssertion(traverser) {
    ...
    let body = ['var expectedOptions = [];',
                ...
                `${traverser.singleIndent()}${traverser.singleIndent()}expect(option.isSelected()).toBe(true);`,
                `${traverser.singleIndent()}});`,
                '',
    ...
    body = ['var expectedOptions = [];',
            ...
            `${traverser.singleIndent()}${traverser.singleIndent()}expect(option.isSelected()).toBe(false);`,
            `${traverser.singleIndent()}});`,
            '}'];
  ...
  emitSelectedAssertion(traverser) {
    ...
    let args = [];
    let result = this.methodHeader(traverser, name, args);
    result += super.methodBody(traverser, `expect(${super.referenceToElement(traverser)}.isSelected()).toBe(true);`);
    result += traverser.newLine();

    name = `${traverser.element.name}ShouldNotBeSelected`;
    result += this.methodHeader(traverser, name, args);
    result += super.methodBody(traverser, `expect(${super.referenceToElement(traverser)}.isSelected()).toBe(false);`);
  ...
  emitSelectedByIndexAssertion(traverser) {
    ...
    let result = this.methodHeader(traverser, name, args);
    result += super.methodBody(traverser,
                               `expect(${super.referenceToElement(traverser)}.get(index).isSelected()).toBe(true);`);
    result += traverser.newLine();

    name = `${traverser.element.name}ByIndexShouldNotBeSelected`;
    result += this.methodHeader(traverser, name, args);
    result += super.methodBody(traverser,
                               `expect(${super.referenceToElement(traverser)}.get(index).isSelected()).toBe(false);`);
  ...
  emitVisibilityAssertion(traverser) {
    ...
    let args = [];
    let result = this.methodHeader(traverser, name, args);
    result += super.methodBody(traverser, `expect(${super.referenceToElement(traverser)}.isDisplayed()).toBe(true);`);
    result += traverser.newLine();

    name = `${traverser.element.name}ShouldNotBeVisible`;
    result += this.methodHeader(traverser, name, args);
    result += super.methodBody(traverser, `expect(${super.referenceToElement(traverser)}.isDisplayed()).toBe(false);`);
  ...
  emitVisibilityByIndexAssertion(traverser) {
    ...
    let result = this.methodHeader(traverser, name, args);
    result += super.methodBody(traverser,
                               `expect(${super.referenceToElement(traverser)}.get(index).isDisplayed()).toBe(true);`);
    result += traverser.newLine();

    name = `${traverser.element.name}ByIndexShouldNotBeVisible`;
    result += this.methodHeader(traverser, name, args);
    result += super.methodBody(traverser,
                               `expect(${super.referenceToElement(traverser)}.get(index).isDisplayed()).toBe(false);`);
