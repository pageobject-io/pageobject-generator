...
class Element {
  ...
  constructor(domElement, parent) {
    ...
    this._types = new Set();
    this._errors = [];
    this._notGenerated = false;
    this._positions = [];
    this.addPosition(domElement);
  }
  ...
  get notGenerated() {
    return this._notGenerated;
  }

  doNotGenerate() {
    this._notGenerated = true;
  }

  addPosition(domElement) {
    if (domElement && domElement.__location) {
      this._positions.push(new Position(domElement.__location.startOffset, domElement.__location.endOffset));
    }
  }
