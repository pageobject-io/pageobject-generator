...
class ComponentDetailsExtractor {
  ...
  findComponentTemplate() {
    let componentMetaString = getComponentAnnotationMetaAsString(this._file);
...
function findComponentTemplate(meta, path) {
  ...
  if (containsTemplate != null) {
    ...
    htmlContent = loadContentFromTemplateUrl(path);
  }
...
function loadContentFromTemplateUrl(typescriptFilePath) {
...
function getComponentAnnotationMetaAsString(file) {
